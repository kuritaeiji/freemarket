class Api::ProductsController < ApplicationController
  before_action(:log_in_user, only: [:index, :traded_index, :solded_index])
  ...
  def traded_index
    products = current_user.products.where(traded: true, solded: false)
    products_as_json = Product.as_json(products)
    render(json: products_as_json)
  end

  def solded_index
    products = current_user.products.where(solded: true)
    products_as_json = Product.as_json(products)
    render(json: products_as_json)
  end
